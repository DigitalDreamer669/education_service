...
export default defineConfig({
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        // registerType: 'autoUpdate' полагается на то, что новый SW сам активируется
        // и зачистит старые кэши. Но у workbox-build skipWaiting/clientsClaim по
        // умолчанию false — без них новый SW после установки уходит в состояние
        // "waiting" и не активируется, пока не закроются ВСЕ вкладки/инстансы
        // страницы (в PWA на телефоне это может не происходить долго). Итог —
        // старый SW продолжает отдавать закэшированный index.html и часть чанков,
        // а часть новых чанков (или сам index.html из adресной строки) грузится уже
        // из свежего деплоя — рассинхронизация версий кэшированных JS-файлов между
        // вкладками/сессиями и, как следствие, непредсказуемо "отваливающиеся" фичи
        // вроде «перемешивать ответы» на части устройств до полного обновления.
        skipWaiting: true,
        clientsClaim: true,
        // Явно чистим кэши предыдущих версий precache при активации нового SW,
        // а не оставляем их висеть в Cache Storage бесконечно.
